...
# --- REFACTOR: Enforce new Volume & Revenue definitions ---
# Volume = HIGHEST_UNIT_SOLD, Revenue = EXT_VAT_VALUE_SOLD
if not lines_all.empty:
    # Ensure numeric types
    for col in ["ext_vat_value_sold", "highest_unit_sold"]:
        if col in lines_all.columns:
            lines_all[col] = pd.to_numeric(lines_all[col], errors='coerce').fillna(0)
            
    if "ext_vat_value_sold" in lines_all.columns:
        lines_all["value_sold_kes"] = lines_all["ext_vat_value_sold"]
    if "highest_unit_sold" in lines_all.columns:
        lines_all["quantity_cases"] = lines_all["highest_unit_sold"]

    # Rebuild orders_all to reflect new revenue definition immediately
    orders_all = lines_all.groupby(["report_date_iso", "sales_rep_id", "customer_id"], as_index=False).agg(
        order_value_kes=("value_sold_kes", "sum"),
        lines_count=("product_code", "nunique")
    )

if not mill_all.empty:
    for col in ["ext_vat_value_sold", "highest_unit_sold"]:
        if col in mill_all.columns:
            mill_all[col] = pd.to_numeric(mill_all[col], errors='coerce').fillna(0)

    if "ext_vat_value_sold" in mill_all.columns:
        mill_all["value_sold_kes"] = mill_all["ext_vat_value_sold"]
    if "highest_unit_sold" in mill_all.columns:
        mill_all["quantity_cases"] = mill_all["highest_unit_sold"]
...
if activity_all.empty:
    ...
    # Alias columns to use new metrics
    if "ext_vat_value_sold" in lines_all.columns:
        lines_all["value_sold_kes"] = lines_all["ext_vat_value_sold"]
    if "highest_unit_sold" in lines_all.columns:
        lines_all["quantity_cases"] = lines_all["highest_unit_sold"]
            ...
            if st.button("✅ Confirm & Process Upload"):
                with st.spinner("Processing..."):
                    ...
                    required_cols = [
                        ...
                        "highest_unit_sold", "ext_vat_value_sold"
                    ]
